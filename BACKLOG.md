...
| BUG-UX-CONFIRM-001 | Bug | P2 | S | Ready | **6 native confirm() blocking dialogs** — app.js×4, email-bills.js×2, subscriptions.js×1. Toast.confirm() exists but uses toast-action-btn pattern (not truly blocking). Proper fix: Bootstrap confirmation modal with Yes/Cancel buttons. Blocks ALL JS execution + terrible on mobile. Est: 2h. (Sprint UI/UX 0445, 2026-02-17) |
...
| FC-179 | Chore | P3 | XS | Ready | **Script defer + Supabase preconnect** — None of the 25 JS `<script>` tags use `defer`. Add `defer` to all non-critical scripts + `<link rel="preconnect">` for Supabase URL in all 11 HTML `<head>` sections. Non-blocking HTML parse = faster perceived load. (30 min) — Sprint Research 2026-02-17 |
