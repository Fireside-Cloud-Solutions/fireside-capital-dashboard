...
**Owner:** Matt Hubacher  
**Last Updated:** 2026-02-17 06:00 EST (Sprint QA 0600: 7 new bugs documented — BUG-TOAST-DUPLICATE-001, BUG-CATEGORIZER-GUARD/TABLE-001, BUG-CSRF-DUPLICATE-001, BUG-CHARTTHEME-DEADCODE-001 FIXED, BUG-DEMO-BILLS-SCHEMA-001, BUG-SESSIONSEC-INLINE-001; plus prior UI/UX bugs logged)
...
| FC-182 | Feature | P1 | S | Ready | **Budget vs Actuals UI Card** — Add BvA section to operations.html (or budget.html): summary row (Total Budget / Actual / Variance), per-category breakdown with Bootstrap progress bars (green<85% / amber 85-100% / red>100%), month selector. Depends on FC-181. (3h) — Sprint Research 2026-02-17 — see reports/budget-vs-actuals-sample-data-research-2026-02-17.md |
...
| BUG-CATEGORIZER-GUARD-001 | Bug | P3 | XS | Ready | **categorizer.js calls `sb.auth.getUser()` without guard** — `checkLearnedPattern()` directly invokes `sb.auth.getUser()` without checking `if (typeof sb === 'undefined')`. If loaded before app.js initializes Supabase, throws ReferenceError. Fix: add `if (typeof sb === 'undefined') return null;` guard. Est: 5 min. (Sprint QA 0600, 2026-02-17) |
